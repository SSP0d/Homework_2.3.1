...
def main() -> None:
    ...
    while True:
        path = input(f'{Fore.YELLOW}Enter path to directory:{Fore.RESET} ')
        ...
        if root_folder.exists():
            ...
            root_folder.is_file()
            print("[-] The file is located at this path")
        

    while True:
        text = input(
            f"{Fore.RED}Confirm the sorting of the files in the directory "
            f"{Fore.YELLOW}'{root_folder.absolute()}' {Fore.RED}(yes/no):{Fore.RESET} ")
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        executor.submit(sort, root_folder)
